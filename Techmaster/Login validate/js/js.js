...
function birthdayVerify() {
  if ($("#birthday").val().trim() == "") {
    ...
    if ($("#birthday").val().substr(0, 4) == new Date().getFullYear() - 18) {
      if (
        Number($("#birthday").val().substr(5, 2)) ==
        new Date().getMonth() + 1
      ) {
        if (Number($("#birthday").val().substr(8, 2)) <= new Date().getDate()) {
          ...
          $(".error.birthday").text("Phải 18 tuổi");
          $("#birthday").addClass("invalid");
          birthdayValid = false;
        }
      } else {
...
function addressVerify() {
  if ($("#address").val().trim() == "") {
    $(".error.address").text("Bạn chưa có địa chỉ.");
...
function phoneVerify() {
  if ($("#phone").val().trim() == "") {
    $(".error.phone").text("Chưa có số điện thoại.");
...
function passwordVerify() {
  if ($("#password").val() == "") {
    $(".error.password").text("Chưa có mật khẩu.");
    $("#password").addClass("invalid");
    passwordValid = false;
  } else {
    if (
      $("#password").val().match(/\W|_/) != null ||
      $("#password").val().match(/[A-Z]/) == null ||
      $("#password").val().match(/\d/) == null ||
      $("#password").val().length < 6 ||
      $("#password").val().length > 20
    ) {
      $(".error.password").text(
        "Password phải có 6-20 kí tự, có ít nhất 1 chữ in hoa và 1 số"
      );
      $("#password").addClass("invalid");
      passwordValid = false;
    } else {
...
function emailVerify() {
  if ($("#email").val().trim() == "") {
    ...
    if (
      $("#email")
        .val()
        .match(/^[\w.]+@([a-zA-Z_]+\.)+[a-zA-Z_]{2,4}$/) == null
    ) {
...
function genderVerify() {
  if (
    ...
    $(".error.gender").text("Chưa có giới tính.");
...
function facebookVerify() {
  if ($("#facebook").val().trim() == "") {
    $(".error.facebook").text("Chưa có tài khoản facebook.");
    ...
    if (
      $("#facebook")
        .val()
        .match(
          /(http[s]?:\/\/(www.facebook|[a-zA-Z]{2}-[a-zA-Z]{2}.facebook|facebook)\.com\/(pages\/[a-zA-Z0-9\.-]+\/[0-9]+|[a-zA-Z0-9\.-]+)[/]?$)|((www.facebook|[a-zA-Z]{2}-[a-zA-Z]{2}.facebook|facebook)\.com\/(pages\/[a-zA-Z0-9\.-]+\/[0-9]+|[a-zA-Z0-9\.-]+)[/]?$)/
        ) == null
    ) {
